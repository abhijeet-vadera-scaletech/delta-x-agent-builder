              ...
              <div className="p-6 space-y-3">
                {providers.map((provider) => (
                  <motion.button
                    ...
                    className={`w-full px-6 py-3 rounded-xl font-semibold text-white shadow-lg hover:shadow-xl transition-all flex items-center justify-center gap-3 bg-gradient-to-r ${getProviderColor(
                      provider
                    )} disabled:opacity-50 disabled:cursor-not-allowed`}
                  >
                    {loading === provider.toLowerCase() ? (
                      <motion.div
                        animate={{ rotate: 360 }}
                        transition={{
                          duration: 1,
